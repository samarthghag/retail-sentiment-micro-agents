...
Mirrors qwen_baseline.py's evaluation protocol (same FPB test split, same prompt
format, same metrics) so the two 7B baselines are directly comparable to each
other and to FinBERT. Mistral-7B-Instruct-v0.3 is gated on HuggingFace — an
HF_TOKEN with access granted is required (see project .env).
"""
...
class MistralBaselineEvaluator:
    """Evaluates Mistral family models on financial benchmark datasets via zero-/few-shot prompting."""

    def __init__(self, model_name: str = "mistralai/Mistral-7B-Instruct-v0.3", load_in_4bit: bool = True):
